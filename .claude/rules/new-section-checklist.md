...
4. **Stega guard** — If the block adds a field whose *value* is compared or used to build a URL (enum-like branches such as `variant`, `layout`, `size`, `alignment`, and identifier strings such as `anchor`), add its name to `STEGA_LOGIC_FIELDS` in `sanity/lib/client.ts`, or clean it with `cleanStega` at the comparison. Visual editing encodes invisible characters into every string, so an unguarded `variant === 'proof'` works in production and silently fails **only in Presentation preview**.
