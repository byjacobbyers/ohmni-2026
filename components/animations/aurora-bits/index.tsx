...
export default function AuroraBits({
  ...
  useEffect(() => {
    ...
    // Mount once; propsRef synced in separate effect above.
    // eslint-disable-next-line react-hooks/exhaustive-deps -- OGL init + rAF loop once
  }, [])

  return (
    <div
      ref={ctnDom}
      className={className ?? 'pointer-events-none absolute inset-0 h-full w-full min-h-px min-w-px'}
    />
  )
}
